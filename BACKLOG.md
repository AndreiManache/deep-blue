...
- [ ] **Favorite/quick-reselect for frequently scanned or logged items.** Andrei: scanned milk two days running and wanted a faster way to pick it again instead of rescanning. The "My Foods" screen (shipped 2026-08-30 — lists/edits every food in your personal `food_observations`) is the natural base for this: the missing piece is a one-tap "log this again" action from that list (needs a quantity prompt, since My Foods stores a per-100g/per-item rate, not a total) rather than a whole separate mechanism.
